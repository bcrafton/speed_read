...
DLLEXPORT int pim(int* x, int* w, int* y, float* lut_var, int* lut_rpr, int* lut_bias, long* metrics, long* confusion, int* block_map, float* adc_state, float* adc_thresh, int adc, int max_rpr, int skip, int R, int B, int C, int NWL, int NBL, int WL, int BL, int sync, int method)
{
  // x = nrow, nwl, wl, xb
  // f = nwl, wl, nbl, bl
  // y = nrow, ncol
  ...
  Params* params = new Params(R, B, C, NWL, NBL, WL, BL, adc, max_rpr, adc_state, adc_thresh, lut_var, lut_rpr, lut_bias, metrics, confusion, sync, method, skip);
