...
def expected_error(params, states, xb, wb, rpr, sar, profile, row, row_avg, adc_thresh, adc_value):
    ...
    ########################################################################
    '''
    pe_mask = 1 - (np.arange(65, dtype=int).reshape(-1, 1) == adc_value)
    ret = np.sum(pe * pe_mask * p)
    '''

    # '''
    PEP = np.sum(p * pe * (e > 0), axis=(1, 2))
    PEN = np.sum(p * pe * (e < 0), axis=(1, 2))
    PE = 2 * np.minimum(PEP, PEN)
    ret = np.sum(PE)
    # '''
    
    '''
    ret = np.sum(p * pe * (np.abs(e) > 0))
    '''
    ...
    if (xb == 0) and (wb == 2) and (rpr == 32) and (sar == 4):
        '''
        print (np.where(1 - pe_mask))
        print (np.sum(pe))
        print (np.sum(pe * pe_mask))
        print (np.sum(1 - pe_mask))
        assert (False)
        '''
        '''
        ref = {}
        ...
        ref['mask'] = pe_mask
        print ()
        print (states, adc_thresh)
        print ()
        ref['conf'] = confusion(THRESH=adc_thresh, RPR=params['max_rpr'], ADC=states, HRS=params['hrs'], LRS=params['lrs'])
        np.save('ref', ref)
        '''
        pass
