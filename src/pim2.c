...
void pim_kernel(data_t* data, state_t* state, dim_t* dim, int rpr, int block, int wl, int bl) {

  clear_vector(state->pdot[block][bl]);
  state->wl_sum[block][bl] = 0;

  int xaddr = (state->r[block] * dim->NWL * dim->WL * 8) + (wl * dim->WL * 8) + (state->wl_ptr[block][bl] * 8) + state->xb[block];  
  
  while ((state->wl_ptr[block][bl] < dim->WL) && (state->wl_sum[block][bl] + data->x[xaddr] <= rpr)) {
    
    if (data->x[xaddr]) {
      state->wl_sum[block][bl] += 1;
      
      for (int adc_ptr=0; adc_ptr<dim->BL; adc_ptr+=8) {
        int bl_ptr = adc_ptr + state->col[block];
        int waddr = (wl * dim->WL * dim->NBL * dim->BL) + (state->wl_ptr[block][bl] * dim->NBL * dim->BL) + (bl * dim->BL) + bl_ptr;
        state->pdot[block][bl][bl_ptr] += data->w[waddr];
      }
    }
    state->wl_ptr[block][bl] += 1;
    xaddr = (state->r[block] * dim->NWL * dim->WL * 8) + (wl * dim->WL * 8) + (state->wl_ptr[block][bl] * 8) + state->xb[block];
  }
...
int pim(int* x, int* w, int* y, float* lut_var, int* lut_rpr, long* metrics, int* block_map, float* adc_state, float* adc_thresh, int adc, int skip, int R, int B, int C, int NWL, int NBL, int WL, int BL)
{
  ...
  data_t  data  = {x, w, y};
  dim_t   dim   = {R, B, C, NWL, NBL, WL, BL};
  array_t array = {adc, adc_state, adc_thresh, lut_var};
  state_t state = {r, next_r, col, xb, wl_ptr, wl_sum, wl_total, pdot, pdot_sum, sat};
  ...
  while (!done) {
    ...
    for (int block=0; block<B; block++) {
      ...
      for (int bl=0; bl<NBL; bl++) {
        ...
        pim_kernel(&data, &state, &dim, rpr, block, wl, bl);
