...
def optimize_rpr(error, mean, delay, valid, threshold):
    ...
    for i in range(xb * wb * step * rpr * sar):
        number_constraint.append( number[i] >= 1 )
    ...
    total_value = value @ cvxpy.multiply(selection, number)

    ##########################################

    knapsack_problem = cvxpy.Problem(cvxpy.Minimize(total_value), select_constraint + number_constraint + [weight_constraint1, weight_constraint2, weight_constraint3, valid_constraint])
