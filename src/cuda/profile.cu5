...
__global__ void gpu_matrix_mult(char *x, char *w, long *y, int X, int Z, int B, int NWL, int WL, int RPR, int RPR_MAX)
{ 
    ...
    for (int wb=0; wb<B; wb++) {
      for (int nwl=0; nwl<NWL; nwl++) {
        ...
        for (int wl=0; wl<WL; wl++) {
          ...
          int y_addr = xb*B*RPR_MAX + wb*RPR_MAX + y_sum;
          y[y_addr] += flag;
          y_sum = (!flag) * y_sum;
          assert (y_sum <= RPR);
          x_sum = (x_sum % RPR);
...
DLLEXPORT int cu_profile(char* x, char* w, long* y, int X, int Z, int B, int NWL, int WL, int RPR, int RPR_MAX)
{
    ...
    cudaMallocHost((void **) &h_y, sizeof(long)*B*B*RPR_MAX);
    ...
    cudaMalloc((void **) &d_y, sizeof(long)*B*B*RPR_MAX);
    ...
    cudaMemcpy(h_y, d_y, sizeof(long)*B*B*RPR_MAX, cudaMemcpyDeviceToHost);
    memcpy(y, h_y, sizeof(long)*B*B*RPR_MAX);
