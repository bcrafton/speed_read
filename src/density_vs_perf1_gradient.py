...
x = np.array(density) * 100
y = total_mac / layer_results['layer_mac']

####################

for i in range(len(x)):
    value = (i + 1) * 12
    # color = '%x%x%x' % (value, value, value)
    color = "#{:02x}{:02x}{:02x}".format(value, value, value)
    print (color)
    plt.scatter(x[i], y[i], marker='+', color=color)
