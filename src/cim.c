...
DLLEXPORT int cim(int8_t* x, int8_t* w, int* y, uint8_t* count, long* error, long* mean, uint8_t* rpr_table, uint64_t* conf, float* value, uint64_t* dist, int size, int max_rpr, int adc, int R, int C, int NWL, int WL, int NBL, int BL) {
        ...
        for (int wb=0; wb<8; wb++) {
          ...
          while (wl_ptr < WL) {
            ...
            for (int bl=0; bl<NBL*BL; bl+=8) {
              ...
              // if (abs(actual - expected) > 0) { error[xb*8 + wb] += 1; }
